...
outfilename = "n2fd_nueapp_paired_stage1"
...
for jentry in range(entries):
    ...
    had_dep_startpos_list = list()
    had_dep_stoppos_list = list()
    had_dep_trkID_list = list()
    had_dep_parentID_list = list()
    had_dep_pdg_list = list()
    had_dep_starttime_list = list()
    had_dep_stoptime_list = list()
    had_edep_list   = list()
    ...
    for containerName, hitSegments in event.SegmentDetectors:
        ...
        for iHit, hitSegment in enumerate(hitSegments):
            ...
            if IsFromPrimaryLep(edep_trkID, trajectories_parentid, PrimaryLepTrackID) == False:
                ...
                had_dep_startpos_list.append(edep_start_x)
                had_dep_startpos_list.append(edep_start_y)
                had_dep_startpos_list.append(edep_start_z)
                had_dep_starttime_list.append(edep_start_t)
                had_dep_stoppos_list.append(edep_stop_x)
                had_dep_stoppos_list.append(edep_stop_y)
                had_dep_stoppos_list.append(edep_stop_z)
                had_dep_stoptime_list.append(edep_stop_t)
                had_dep_trkID_list.append(edep_trkID)
                had_dep_parentID_list.append(edep_parentID)
                had_dep_pdg_list.append(edep_pdg)
    ...
    nNonLepEdeps[0] = len(had_edep_list)
    ...
    while tot_nd_throw < max_nd_throws:
        ...
        if ( IsInNDFV( throwVtxX_nd[0] - NDLAr_OnAxis_offset[0], throwVtxY_nd[0] - NDLAr_OnAxis_offset[1], throwVtxZ_nd[0] - NDLAr_OnAxis_offset[2]) ):
            ...
            if (ndrandthrowresulthad.containresult[0][0][0] != 0):
                ...
                # Repeat for hadronic start points
                # need for numu-nue paired dataset
                geoEff.setHitSegEdeps(had_edep_list)
                geoEff.setHitSegPoss(had_dep_startpos_list)
                ndrandthrowresulthad_start = geoEff.getNDContainment4RandomThrowX()

                # Repeat for hadronic stop points
                # need for numu-nue paired dataset
                geoEff.setHitSegEdeps(had_edep_list)
                geoEff.setHitSegPoss(had_dep_stoppos_list)
                ndrandthrowresulthad_stop = geoEff.getNDContainment4RandomThrowX()
                ...
                if nd_lep_contained_prob_nonecc[0] >= nd_contained_mu_cut or nd_lep_tracker_prob_nonecc[0] >= nd_tracker_mu_cut:
                    ...
                    # Repeat for start and stop points of non-primary-lepton deposits
                    had_startposdep_ndorig_matrix = geoEff.move2ndorigin(ndrandthrowresulthad_start.thrownEdepspos[0]) # returns Eigen::Matrix3Xf
                    had_stopposdep_ndorig_matrix = geoEff.move2ndorigin(ndrandthrowresulthad_stop.thrownEdepspos[0]) # repeat for stop points
                    ...
                    had_startposdep_fdorig_matrix = geoEff.getn2fEarthCurvatureCorr(had_startposdep_ndorig_matrix, beamLineRotation)
                    had_stopposdep_fdorig_matrix = geoEff.getn2fEarthCurvatureCorr(had_stopposdep_ndorig_matrix, beamLineRotation) # repeat for stop points
                    # Also apply to the lepton momentum vector as we need this to generate electrons in stage 2
                    # right hand rule: -2*abs(beamLineRotation) around x axis, clockwise 2*abs(beamLineRotation)
                    ecc_rot_axis = [1, 0, 0]
                    ecc_rot_vec = np.multiply(ecc_rot_axis, -2*abs(beamLineRotation))
                    ecc_rot = R.from_rotvec(ecc_rot_vec)
                    lep_p = ecc_rot.apply(lep_p)

                    ###########################################################################################################################################################
                    # Summary of what should be done at
                    # numu-nue paired dataset stage 1:
                    #
                    #   1) numu deposits fd side info is discarded. All the rest is stored (same as numu disappearance case), i.e., from Genie up to nd selected (nd_*_non_ecc)
                    #   2) At nd ecc step, non-muon energy deposits are tagged for collation with electron deposition info to be used in FD (stage 2)
                    #   3) At nd ecc step (i.e., nd selected + earth curvature correction applied), extract lepton direction at this nd ecc step,
                    #      its total energy in MeV (Genie_final_lep_p3_MeV), and vtx position (which is now at origin): for particle gun electron generation + edepsim
                    #
                    ############################################################################################################################################################
                    ...
                    larbath_deps_start_y_cm[:nEdeps[0]] = np.array(all_dep_startpos_list[1::3], dtype=np.float32)
                    larbath_deps_start_z_cm[:nEdeps[0]] = np.array(all_dep_startpos_list[2::3], dtype=np.float32)
                    ...
                    # Selected nd + earth curvature correction for non lepton deposits
                    # for collation with electron in nue paired data
                    nd_nonlepdeps_start_x_cm_ecc[:nNonLepEdeps[0]] = np.array(had_startposdep_fdorig_matrix[0,:], dtype=np.float32)
                    nd_nonlepdeps_start_y_cm_ecc[:nNonLepEdeps[0]] = np.array(had_startposdep_fdorig_matrix[1,:], dtype=np.float32)
                    nd_nonlepdeps_start_z_cm_ecc[:nNonLepEdeps[0]] = np.array(had_startposdep_fdorig_matrix[2,:], dtype=np.float32)
                    nd_nonlepdeps_stop_x_cm_ecc[:nNonLepEdeps[0]]  = np.array(had_stopposdep_fdorig_matrix[0,:], dtype=np.float32)
                    nd_nonlepdeps_stop_y_cm_ecc[:nNonLepEdeps[0]]  = np.array(had_stopposdep_fdorig_matrix[1,:], dtype=np.float32)
                    nd_nonlepdeps_stop_z_cm_ecc[:nNonLepEdeps[0]]  = np.array(had_stopposdep_fdorig_matrix[2,:], dtype=np.float32)
                    # LArBath info for nonlepton deposits
                    nonlepdeps_trkID[:nNonLepEdeps[0]] = np.array(had_dep_trkID_list, dtype=np.int32)
                    nonlepdeps_parentID[:nNonLepEdeps[0]] = np.array(had_dep_parentID_list, dtype=np.int32)
                    nonlepdeps_pdg[:nNonLepEdeps[0]] = np.array(had_dep_pdg_list, dtype=np.int32)
                    nonlepdeps_E_MeV[:nNonLepEdeps[0]] = np.array(had_edep_list, dtype=np.float32)
                    nonlepdeps_start_t_us[:nNonLepEdeps[0]] = np.array(had_dep_starttime_list, dtype=np.float32)
                    nonlepdeps_stop_t_us[:nNonLepEdeps[0]] = np.array(had_dep_stoptime_list, dtype=np.float32)
                    larbath_nonlepdeps_start_x_cm[:nNonLepEdeps[0]] = np.array(had_dep_startpos_list[::3], dtype=np.float32) # every 3 element: x list
                    larbath_nonlepdeps_start_y_cm[:nNonLepEdeps[0]] = np.array(had_dep_startpos_list[1::3], dtype=np.float32)
                    larbath_nonlepdeps_start_z_cm[:nNonLepEdeps[0]] = np.array(had_dep_startpos_list[2::3], dtype=np.float32)
